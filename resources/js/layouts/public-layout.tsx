...
export default function PublicLayout({
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900 dark:bg-[#0b1021] dark:text-white">
            <nav className="sticky top-0 z-20 border-b border-slate-200/70 bg-white/80 backdrop-blur dark:border-white/10 dark:bg-[#0f162e]/80">
                        ...
                        {links.map((link) => (
                            <Link
                                ...
                                className={`rounded-full px-3 py-2 transition ${
                                    link.active
                                        ? 'bg-slate-900 text-white dark:bg-white dark:text-[#0b1021]'
                                        : 'text-slate-700 hover:bg-slate-100 dark:text-blue-100 dark:hover:bg-white/10'
                                }`}
                            >
                                {link.label}
                            </Link>
                    ...
                    <div className="flex items-center gap-3 text-sm">
                        {auth.user ? (
                            <div className="relative">
                                ...
                                {openMenu && (
                                    <div className="absolute right-0 mt-2 w-44 rounded-xl border border-slate-200 bg-white text-sm shadow-lg ring-1 ring-black/5 dark:border-white/10 dark:bg-[#0f172a]">
                                        <Link
                                            href="/dashboard"
